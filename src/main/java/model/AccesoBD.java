...
/**
 * Clase que se encarga de gestionar la conexión con la base de datos y de
 * realizar las consultas necesarias para la aplicación.
 */
public class AccesoBD {
    ...
    /**
     * Método que se encarga de cerrar la conexión con la base de datos una vez que
     * ya no se necesite.
     */
    ...
    /**
     * Método que se encarga de comprobar si el login introducido por el usuario es
     * correcto.
     * 
    ...
    public Object[] comprobarLogin(Connection c, String login) throws SQLException {
        ...
        String query = "SELECT id, password FROM miembros WHERE nombre = '" + login + "'"; // Consulta para obtener el
                                                                                           // id y la contraseña del
                                                                                           // usuario
        ...
        while (resultSet.next()) { // Mientras haya resultados en el ResultSet
            ...
            resultado[1] = resultSet.getString("password"); // Guardamos la contraseña del usuario en la segunda
                                                            // posición del array
        }
    ...
    public ArrayList<String[]> hacerConsultaTablaCuenta(Connection c) throws SQLException {
        ...
        try {
            ...
            while (resultados.next()) { // Mientras haya resultados en el ResultSet
                ...
                try {
                    pstmt2 = con.prepareStatement(query2); // Creamos un objeto de tipo PreparedStatement con la
                                                           // consulta
                    ...
                    while (resultados2.next()) { // Mientras haya resultados en el ResultSet
                        ...
                        fila[0] = resultados.getString("nombre"); // Guardamos el nombre del personaje en la primera
                                                                  // posición de la fila
                        fila[1] = resultados.getString("raza"); // Guardamos la raza del personaje en la segunda
                                                                // posición de la fila
                        fila[2] = resultados.getString("clase"); // Guardamos la clase del personaje en la tercera
                                                                 // posición de la fila

                        int partidaId = resultados2.getInt("partida_id"); // Guardamos el id de la partida en una
                                                                          // variable

                        fila[4] = String.valueOf(resultados2.getInt("nivel")); // Guardamos el nivel del personaje en la
                                                                               // cuarta posición de la fila
                        fila[5] = String.valueOf(resultados2.getInt("salud")); // Guardamos la salud del personaje en la
                                                                               // quinta posición de la fila
                        fila[6] = String.valueOf(resultados2.getInt("fuerza")); // Guardamos la fuerza del personaje en
                                                                                // la sexta posición de la fila
                        fila[7] = String.valueOf(resultados2.getInt("destreza")); // Guardamos la destreza del personaje
                                                                                  // en la séptima posición de la fila
                        fila[8] = String.valueOf(resultados2.getInt("sabiduria")); // Guardamos la sabiduría del
                                                                                   // personaje en la octava posición de
                                                                                   // la fila
                        fila[9] = String.valueOf(resultados2.getInt("carisma")); // Guardamos el carisma del personaje
                                                                                 // en la novena posición de la fila
                        fila[10] = String.valueOf(resultados2.getInt("inteligencia")); // Guardamos la inteligencia del
                                                                                       // personaje en la décima
                                                                                       // posición de la fila
                        fila[11] = String.valueOf(resultados2.getInt("constitucion")); // Guardamos la constitución del
                                                                                       // personaje en la undécima
                                                                                       // posición de la fila

                        try {
                            pstmt3 = con.prepareStatement(query3); // Creamos un objeto de tipo PreparedStatement con la
                                                                   // consulta
                            ...
                            if (resultados3.next()) { // Si hay resultados en el ResultSet
                                String nombrePartida = resultados3.getString("nombre"); // Guardamos el nombre de la
                                                                                        // partida en una variable
                                if (nombrePartida != null && !nombrePartida.trim().isEmpty()) { // Si el nombre de la
                                                                                                // partida no es nulo ni
                                                                                                // está vacío
                                    fila[3] = nombrePartida; // Guardamos el nombre de la partida en la tercera posición
                                                             // de la fila
    ...
    /**
     * Método para hacer una consulta a la base de datos y obtener los niveles de un
    ...
    public ArrayList<String[]> hacerConsultaNivelesPersonaje(Connection c, int personajeId, int partidaId)
        ...
        try {
            ...
            while (resultados.next()) { // Mientras haya resultados en el ResultSet
                ...
                fila[0] = String.valueOf(resultados.getString("nivel")); // Guardamos el nivel del personaje en la
                                                                         // primera posición de la fila
                fila[1] = String.valueOf(resultados.getString("salud")); // Guardamos la salud del personaje en la
                                                                         // segunda posición de la fila
                fila[2] = String.valueOf(resultados.getString("fuerza")); // Guardamos la fuerza del personaje en la
                                                                          // tercera posición de la fila
                fila[3] = String.valueOf(resultados.getString("destreza")); // Guardamos la destreza del personaje en la
                                                                            // cuarta posición de la fila
                fila[4] = String.valueOf(resultados.getString("sabiduria")); // Guardamos la sabiduría del personaje en
                                                                             // la quinta posición de la fila
                fila[5] = String.valueOf(resultados.getString("carisma")); // Guardamos el carisma del personaje en la
                                                                           // sexta posición de la fila
                fila[6] = String.valueOf(resultados.getString("inteligencia")); // Guardamos la inteligencia del
                                                                                // personaje en la séptima posición de
                                                                                // la fila
                fila[7] = String.valueOf(resultados.getString("constitucion")); // Guardamos la constitución del
                                                                                // personaje en la octava posición de la
                                                                                // fila
    ...
    /**
     * Método para hacer una consulta a la base de datos y obtener los detalles de
     * una partida en concreto.
     * 
     * @param c                   Conexión con la base de datos.
    ...
    public ArrayList<String[]> hacerConsultaTablaDetallesPartida(Connection c, String nombrePartida,
        ...
        try (PreparedStatement pstmt1 = c.prepareStatement(query)) { // Creamos un objeto de tipo PreparedStatement con
                                                                     // la consulta
                ...
                while (resultados.next()) { // Mientras haya resultados en el ResultSet
                    ...
                    try (PreparedStatement pstmt2 = c.prepareStatement(query2)) {
                        ...
                        try (ResultSet resultados2 = pstmt2.executeQuery()) {
                            while (resultados2.next()) { // Mientras haya resultados en el ResultSet
                                int personajeId = resultados2.getInt("personaje_id"); // Guardamos el id del personaje
                                                                                      // en una variable
                                ...
                                fila[3] = String.valueOf(resultados2.getInt("nivel")); // Guardamos el nivel del
                                                                                       // personaje en la cuarta
                                                                                       // posición de la fila
                                ...
                                try (PreparedStatement pstmt3 = c.prepareStatement(query3)) {
                                    ...
                                    try (ResultSet resultados3 = pstmt3.executeQuery()) {
                                        if (resultados3.next()) { // Si hay resultados en el ResultSet
                                            fila[0] = resultados3.getString("nombre"); // Guardamos el nombre del
                                                                                       // personaje en la primera
                                                                                       // posición de la fila
                                            fila[1] = resultados3.getString("raza"); // Guardamos la raza del personaje
                                                                                     // en la segunda posición de la
                                                                                     // fila
                                            fila[2] = resultados3.getString("clase"); // Guardamos la clase del
                                                                                      // personaje en la tercera
                                                                                      // posición de la fila
    ...
    public ArrayList<String[]> hacerConsultaTablaPartidas(Connection c, int personaje_id) throws SQLException {
        ...
        try (
            ...
            while (resultados.next()) { // Mientras haya resultados en el ResultSet
                ...
                fila[1] = resultados.getString("nombre"); // Guardamos el nombre de la partida en la segunda posición de
                                                          // la fila
                fila[2] = resultados.getString("ambientacion"); // Guardamos la ambientación de la partida en la tercera
                                                                // posición de la fila
    ...
    /**
     * Metodo para crear un personaje en la base de datos con los datos introducidos
     * por el usuario.
     * 
     * @param c          Conexión con la base de datos.
    ...
    public boolean crearPersonaje(Connection c, String nombre, String sexo, String raza, String clase, int miembro_id) {
        ...
        try (PreparedStatement pstmt = c.prepareStatement(queryComprobacion)) {
            ...
            if (!resultados.next()) { // Si no hay resultados en el ResultSet
                try (PreparedStatement pstmt2 = c.prepareStatement(query)) { // Creamos un objeto de tipo
                                                                             // PreparedStatement con la consulta
    ...
    /**
     * Método para crear un miembro en la base de datos con los datos introducidos
     * por el usuario.
     * 
     * @param c              Conexión con la base de datos.
    ...
    /**
     * Método para crear una partida en la base de datos con los datos introducidos
     * por el usuario.
     * 
     * @param c            Conexión con la base de datos.
    ...
    /**
     * Método para editar partida en la base de datos con los datos introducidos por
     * el usuario.
     * 
     * @param c                        Conexión con la base de datos.
    ...
    public void editarPartida(Connection c, String nuevoNombrePartida, String nuevaAmbientacionPartida, int partidaId)
        ...
        try {
            ...
            pstmt.setString(2, nuevaAmbientacionPartida); // Establecemos la nueva ambientación de la partida en la
                                                          // consulta
        ...
        try {
            ...
            try {
                ...
                for (int i = 4; i <= 10; i++) { // Bucle para recorrer las columnas de la tabla
                    pstmt2.setInt(i, rnd.nextInt(resultados.getInt(i), resultados.getInt(i) + 101)); // Establecemos un
                                                                                                     // valor aleatorio
                                                                                                     // entre el valor
                                                                                                     // actual y el
                                                                                                     // valor actual +
    ...
    public int[][] subidaNivelPersonajeDatos(Connection c, int personajeId, int partidaId) throws SQLException {
        ...
        try {
            ...
            while (resultados.next()) {
                ...
                datosSubidaNivel[contadorFila][0] = resultados.getInt(1); // Establecemos el nivel del personaje en la
                                                                          // consulta
    ...
    public int devolverPersonajeId(Connection c, String nombrePersonaje, int miembroId) throws SQLException {
        ...
        try {
            ...
            System.out.println("Con nombrePersonaje: " + nombrePersonaje + " y miembroId: " + miembroId); // Mostramos
                                                                                                          // el nombre
                                                                                                          // del
                                                                                                          // personaje y
                                                                                                          // el id del
            ...
            if (resultados.next()) { // Si hay resultados en el ResultSet
                ...
                System.out.println("personajeId encontrado: " + personajeId); // Mostramos el id del personaje por
                                                                              // consola
            } else {
    ...
    public ArrayList<String> devolverPersonajesMiembro(Connection con) throws SQLException {
        ...
        String query = "SELECT nombre FROM personajes WHERE miembro_id = ?"; // Consulta para obtener los personajes de
                                                                             // un miembro
        System.out.println(query); // Mostramos la consulta por consola
        ...
        try {
            ...
            if (resultados != null) { // Si hay resultados en el ResultSet
                try {
                    ...
                    System.err.println("Error closing ResultSet: " + e.getMessage()); // Mostramos un mensaje de error
                                                                                      // por consola
                }
            }
            if (pstmt != null) { // Si el PreparedStatement no es nulo
        ...
        try {
            ...
            if (pstmt != null) { // Si el PreparedStatement no es nulo
                try {
                    ...
                    System.err.println("Error closing PreparedStatement: " + e.getMessage()); // Mostramos un mensaje de
                                                                                              // error por consola
                }
            }
            if (con != null) { // Si la conexión no es nula
    ...
    /**
     * Método para devolver el nombre de las partidas de un game master en la base
     * de datos.
     * 
     * @param c Conexión con la base de datos.
    ...
    /**
     * Método para devolver el ID de una partida del game master en la base de
     * datos.
     * 
     * @param c             Conexión con la base de datos.
     * @param nombrePartida Nombre de la partida.
    ...
    /**
     * Método para devolver el personaje seleccionado de un miembro en la base de
     * datos.
     * 
     * @param c               Conexión con la base de datos.
     * @param nombrePersonaje Nombre del personaje.
    ...
    public Object[] devolverPersonajeSeleccionadoMiembro(Connection c, String nombrePersonaje) throws SQLException {
        ...
        try {
            ...
            if (resultados.next()) { // Si hay resultados en el ResultSet
                datosPersonajeSeleccionado[0] = (Integer) resultados.getInt(1); // Guardamos el id del personaje en la
                                                                                // primera posición del array
                datosPersonajeSeleccionado[1] = (String) resultados.getString(2); // Guardamos la raza del personaje en
                                                                                  // la segunda posición del array
                datosPersonajeSeleccionado[2] = (String) resultados.getString(3); // Guardamos el sexo del personaje en
                                                                                  // la tercera posición del array
            }
